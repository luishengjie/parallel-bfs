...
def process_pennant_batch(in_pennant_lst,d,target):
    final_flag = False
    nodes_list = []
    for k_baglist in in_pennant_lst:
        nodes, flag = process_pennant(k_baglist,d,target)
        nodes_list += nodes
        if flag:
            final_flag = True
    return nodes_list, final_flag



def get_next_bag(cur_bag, d, target):
    """ Parallel implementation tp get the next bag
    """
    nodes_list = []
    final_flag = False
    ...
    # num_cpu = 2
    n = math.ceil(len(baglist)/num_cpu)
    baglist_batch = [baglist[i:i+n] for i in range(0, len(baglist), n)]

    with Pool(num_cpu) as pool:
        results = pool.map(partial(process_pennant_batch, d=d, target=target), baglist_batch)
        nodes_list = [x for (x,y) in results]
        nodes_list = list(np.concatenate(nodes_list).ravel())
        final_flag = False
        for (x,y) in results:
            if y:
                final_flag = True  
...
if __name__ == '__main__':
    ...
    find_node=pbfs(graph.root,target=998)
